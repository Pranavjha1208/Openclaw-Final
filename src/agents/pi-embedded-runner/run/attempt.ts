  ...
  try {
    ...
    const creatorFromConfig =
      params.config && sessionAgentId
        ? resolveAgentIdentity(params.config, sessionAgentId)?.creator?.trim()
        : undefined;
    const creatorFromFile = loadAgentIdentity(effectiveWorkspace)?.creator?.trim();
